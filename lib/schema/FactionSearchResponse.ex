defmodule Torngen.Client.Schema.FactionSearchResponse do
  ...
  def parse(%{} = data) do
    %__MODULE__{
      search:
        data
        |> Map.get("search")
        |> Torngen.Client.Schema.parse({:array, {:ref, Torngen.Client.Schema.FactionSearch}}),
      _metadata:
        data
        |> Map.get("_metadata")
  ...
  defp validate_key?(:search, value) do
    Torngen.Client.Schema.validate?(value, {:array, {:ref, Torngen.Client.Schema.FactionSearch}})
  end
