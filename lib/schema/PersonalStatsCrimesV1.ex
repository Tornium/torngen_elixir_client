defmodule Torngen.Client.Schema.PersonalStatsCrimesV1 do
  ...
  def parse(%{} = data) do
    %__MODULE__{
      ...
      sell_illegal_goods:
        data |> Map.get("sell_illegal_goods") |> Torngen.Client.Schema.parse({:static, :integer}),
      other: data |> Map.get("other") |> Torngen.Client.Schema.parse({:static, :integer}),
      organized_crimes:
        data |> Map.get("organized_crimes") |> Torngen.Client.Schema.parse({:static, :integer}),
      ...
      drug_deals:
        data |> Map.get("drug_deals") |> Torngen.Client.Schema.parse({:static, :integer}),
      computer: data |> Map.get("computer") |> Torngen.Client.Schema.parse({:static, :integer}),
      auto_theft:
        data |> Map.get("auto_theft") |> Torngen.Client.Schema.parse({:static, :integer})
    }
  end
