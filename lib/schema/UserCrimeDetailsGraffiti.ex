defmodule Torngen.Client.Schema.UserCrimeDetailsGraffiti do
  ...
  def parse(%{} = data) do
    %__MODULE__{
      most_graffiti_simultaneously:
        data
        |> Map.get("most_graffiti_simultaneously")
        |> Torngen.Client.Schema.parse({:static, :integer}),
      most_graffiti_in_one_area:
        data
        |> Map.get("most_graffiti_in_one_area")
        |> Torngen.Client.Schema.parse({:static, :integer}),
      graffiti_removed:
        data |> Map.get("graffiti_removed") |> Torngen.Client.Schema.parse({:static, :integer}),
      cost_to_city:
        data |> Map.get("cost_to_city") |> Torngen.Client.Schema.parse({:static, :integer}),
