defmodule Torngen.Client.Schema.UserCrimeDetailsHustling do
  ...
  def parse(%{} = data) do
    %__MODULE__{
      total_audience_gathered:
        data
        |> Map.get("total_audience_gathered")
        |> Torngen.Client.Schema.parse({:static, :integer}),
      shill_money_collected:
        data
        |> Map.get("shill_money_collected")
        |> Torngen.Client.Schema.parse({:static, :integer}),
      pickpocket_money_collected:
        data
        |> Map.get("pickpocket_money_collected")
        |> Torngen.Client.Schema.parse({:static, :integer}),
      biggest_money_won:
        data |> Map.get("biggest_money_won") |> Torngen.Client.Schema.parse({:static, :integer})
    }
  end
